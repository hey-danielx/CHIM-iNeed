...
Bool Function AutoConsume(Actor Target, Formlist Formlist1, FormList FormList2 = None, FormList FormList3 = None, FormList FormList4 = None, Bool Remove = False, Bool Text = True)
...
Bool Function AutoEat(Actor Target, Bool Text = True, Bool Remove = False)
	If _SNCannibalToggle.GetValue() as Int == 1
		If FoodPriority == 0 && AutoConsume(Target, _SNFood_RawList, _SNFood_HeavyList, _SNFood_MedList, _SNFood_SoupList, Remove, Text)
			Return True
		ElseIf AutoConsume(Target, _SNFood_RawList, _SNFood_SoupList, _SNFood_MedList, _SNFood_HeavyList, Remove, Text)
			Return True
		EndIf
	Else
		If FoodPriority == 0 && AutoConsume(Target, _SNFood_HeavyList, _SNFood_MedList, _SNFood_SoupList, _SNFood_LightList, Remove, Text)
			Return True
		ElseIf AutoConsume(Target, _SNFood_LightList, _SNFood_SoupList, _SNFood_MedList, _SNFood_HeavyList, Remove, Text)
			Return True
		EndIf
	EndIf
	Return False
...
Function AutoDrink(Actor Target, Bool Text = True)
	AutoDrinking = True
	AutoConsume(Target, _SNFood_WaterList, _SNFood_DrinkNoAlcList, _SNFood_DrinkList, _SNFood_DrinkSnowList, false, Text)
	AutoDrinking = False
EndFunction

Function AutoDrinkNoAlc(Actor Target, Bool Text = True)
	AutoDrinking = True
	AutoConsume(Target, _SNFood_WaterList, _SNFood_DrinkNoAlcList, _SNFood_DrinkSnowList, None, false, Text)
	AutoDrinking = False
EndFunction

Bool Function AutoEatHorse(Actor Target, Bool Text = True)
	If AutoConsume(Target, _SNFood_HorseList, None, None, None, true, Text)
		Return True
	EndIf
	Return False
EndFunction
...
;====================================================================================			;Public needs API (CHIM / other mods)

Int Function GetHungerPenaltyInt()
	Return _SNHungerPenalty.GetValue() as Int
EndFunction

Int Function GetThirstPenaltyInt()
	Return _SNThirstPenalty.GetValue() as Int
EndFunction

Int Function GetFatiguePenaltyInt()
	Return _SNFatiguePenalty.GetValue() as Int
EndFunction

Int Function GetFollowerNeedsType()
	Return _SNFollowerNeedsToggle.GetValue() as Int
EndFunction

Bool Function AreFollowerNeedsEnabled()
	Return GetFollowerNeedsType() > 0
EndFunction

String Function GetHungerLabel()
	Int Penalty = GetHungerPenaltyInt()
	If Penalty == -2
		Return "well sated"
	ElseIf Penalty == -1
		Return "sated"
	ElseIf Penalty == 0
		Return "a bit hungry"
	ElseIf Penalty == 1
		Return "mildly hungry"
	ElseIf Penalty == 2
		Return "moderately hungry"
	EndIf
	Return "severely hungry"
EndFunction

String Function GetThirstLabel()
	Int Penalty = GetThirstPenaltyInt()
	If Penalty == -2
	...
	ElseIf Penalty == 2
		Return "moderately thirsty"
	EndIf
	Return "severely thirsty"
EndFunction

String Function GetFatigueLabel()
	Int Penalty = GetFatiguePenaltyInt()
	If Penalty == -2
		Return "well rested"
	ElseIf Penalty == -1
		Return "rested"
	...
	ElseIf Penalty == 1
		Return "mildly tired"
	ElseIf Penalty == 2
		Return "moderately tired"
	EndIf
	Return "severely tired"
EndFunction

String Function GetPlayerNeedsSummary()
	Return "Hunger: " + GetHungerLabel() + ". Thirst: " + GetThirstLabel() + ". Fatigue: " + GetFatigueLabel() + "."
EndFunction
...
Bool Function ChimNeedCommentReady(String asActorName, String asNeed)
	If !ChimCommentsEnabled || asActorName == "" || asNeed == ""
		Return False
	EndIf
	EnsureChimNeedCommentStorage()
	String KeyName = asActorName + "|" + asNeed
	Float Now = Utility.GetCurrentGameTime()
	Int i = 0
	While i < ChimNeedCommentKeys.Length
		If ChimNeedCommentKeys[i] == KeyName
			If ((Now - ChimNeedCommentTimes[i]) * 24.0) < ChimCommentCooldownHours
				Return False
			EndIf
			Return True
		EndIf
...
Function ChimMarkNeedComment(String asActorName, String asNeed)
...
Function RegisterChimEvents()
	If SKSEVersion > 0.0 && Game.GetModByName("AIAgent.esp") != 255
...
Event OnCHIMCommand(String npcName, String command, String parameter)
	If command == "ExtCmdINeed_Eat"
		SendModEvent("_SN_ChimEat", npcName)
	ElseIf command == "ExtCmdINeed_Drink"
		SendModEvent("_SN_ChimDrink", npcName)
	EndIf
EndEvent
...
Function Maintenance()
	...
	;If _SNDiseaseToggle.GetValue() as Int == 1								disease stuff
	...
	If fModVersion
		...
		EndIf
		If fModVersion < 1.89							;CHANGE VERSION NUMBER WHEN UPDATING
			ModUpdated = True
			Restart()
			Debug.Notification(UpdateText + "v1.90 Alpha 1")	;CHANGE VERSION NUMBER WHEN UPDATING
		EndIf
